#!/usr/bin/env python3
"""
Icarus Fandom Wiki Database Scraper - FIXED VERSION
Fixes ingredient name truncation bug (e.g., "12-Gauge" → "-Gauge")
...
def clean_ingredient_name(raw_name):
    """
    Clean ingredient name without destroying item names that contain numbers.
    
    FIXED: Prevents "12-Gauge Buckshot Shell" from becoming "-Gauge Buckshot Shell"
    
    Examples:
        "5x Stone" → "Stone"
        "Stone (5)" → "Stone"
        "12-Gauge Buckshot Shell" → "12-Gauge Buckshot Shell" ✓
        "7.62mm Round" → "7.62mm Round" ✓
        ".308 Rifle Ammo" → ".308 Rifle Ammo" ✓
    """
    ...
    # Remove quantity patterns at the START of the string only
    # Matches patterns like "5 Stone", "10x Iron", etc.
    name = re.sub(r'^(\d+\s*x?\s*)', '', name, flags=re.IGNORECASE)
    
    # Remove quantity in parentheses anywhere: "(5)", "(10)", etc.
    name = re.sub(r'\s*\(\d+\)', '', name)
    
    # Remove trailing quantities: "Stone 5", "Iron 10", etc.
    name = re.sub(r'\s+\d+$', '', name)
                    ...
                    for i in range(len(cells) - 1):
                        ...
                        for j in range(i, min(i + 2, len(cells))):
                            ...
                            if quantity_match:
                                ...
                                raw_item_name = item_cell.get_text(strip=True)
                                
                                # FIXED: Use improved cleaning function
                                item_name = clean_ingredient_name(raw_item_name)
                                
                                # Validate: must be reasonable length and have valid quantity
                                if len(item_name) > 2 and quantity > 0 and quantity < 10000:
                                    # Additional validation: skip if it's just a number or category label
                                    if not item_name.isdigit() and not item_name.lower() in ['quantity', 'amount', 'count', 'number']:
...
def extract_item_data(page_url, quiet=True):
    ...
    item_data = {
        ...
        "research_cost": None,
        "purchase_cost": None,
        "base_recipe": None  # NEW: Store base recipe separately for talent system
    }
    ...
    try:
        ...
        # NEW: Store base_recipe separately for talent calculations
        if ingredients or crafted_at != "Unknown":
            item_data['base_recipe'] = {
                "ingredients": ingredients,
                "crafted_at": crafted_at
            }
            
            # Also keep in old format for backwards compatibility
...
def categorize_items(items):
    ...
    for item in items:
        ...
        # Ammunition
        if not categorized and any(word in name_lower for word in ['bullet', 'shell', 'arrow', 'ammo', 'cartridge', 'round', 'gauge']):
...
def scrape_all_items(output_dir="icarus_data", max_workers=5):
    ...
    print("="*70)
    print("  ICARUS FANDOM WIKI SCRAPER - FIXED VERSION")
    print("  ✓ Fixed ingredient name truncation bug")
    print("="*70)
    ...
    index = {
        ...
        "fixes": ["Ingredient name preservation (12-Gauge Buckshot Shell)", "base_recipe support for talent system"]
    }
    ...
    summary = {
        ...
        "scraper_version": "2.0-fixed",
        "fixes_applied": [
            "Fixed ingredient name truncation (preserves '12-Gauge Buckshot Shell')",
            "Added base_recipe field for talent system support",
            "Improved number preservation in item names (e.g., 7.62mm, .308)"
        ],
        "structure": {
    ...
    print(f"\n🔧 FIXES APPLIED:")
    print(f"  ✓ Ingredient names preserved (12-Gauge Buckshot Shell)")
    print(f"  ✓ Numbers in item names kept (7.62mm Round, .308 Rifle Ammo)")
    print(f"  ✓ Added base_recipe field for talent calculations")
    print(f"\n📁 File Structure:")
    print(f"  icarus_data/")
...
if __name__ == "__main__":
    print("\nICARUS FANDOM WIKI SCRAPER - FIXED VERSION")
    print("This will scrape items from icarus.fandom.com")
    print("\n✓ Fixes ingredient name truncation bug")
    print("✓ Preserves numbers in item names (12-Gauge, 7.62mm, etc.)")
